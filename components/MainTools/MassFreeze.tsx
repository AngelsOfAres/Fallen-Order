...
export default function MassFreeze() {
  ...
  const massFreeze = async () => {
    ...
    try {
      ...
      toast.success(`${freeze} Successful! Total Addresses: ${addressList.length}`, {
        id: 'txn',
        duration: 5000
      })
      setLoading(false)
      ...
      toast.error(`Oops! Mass ${freeze} failed. Please verify asset IDs and try again...`, { id: 'txn' })
    }
  }
  ...
  return (
    <Box className={boxGlow} p='6px' m='20px' minW='300px' maxW='480px' bg="black" borderRadius="20px">
      <div className="pt-5 sm:px-6 relative">
        <Text className='hFont' textColor={medColor} textAlign="center">Mass Freeze</Text>
      </div>
      <div className="mx-5 pb-1 pt-3">
        <Center>
          <VStack mb={6} spacing='12px' w='fit-content'>
              <Text textColor={lightColor} className='whitespace-nowrap'>{freeze ? 'Freeze' : 'Unfreeze'}</Text>
              <Switch defaultChecked={true} size='lg' colorScheme={baseColor} css={{"& .chakra-switch__thumb": {backgroundColor: "black" }}} onChange={() => setFreeze(freeze ? false : true)} />
          </VStack>
        </Center>
        <form onSubmit={handleSubmit}>            
          ...
          {!loading ?
            <Center my={6}>
              <FullGlowButton text={freeze? 'Freeze!' : 'Unfreeze!'} disabled={addressList.length === 0 || addressList.includes('')} onClick={handleSubmit}/>
            </Center>
          : 
            <Center my={2}>
              <VStack pb={4}>
